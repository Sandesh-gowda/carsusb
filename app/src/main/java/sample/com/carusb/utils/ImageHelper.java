    ...
    private static void initialiseStaticBlock() {
        if (isPicassoEnabled) {
            ...
            try {
                ...
                okHttpClient.setCache(new Cache(MyApplication.getInstance().getCacheDir(), cacheSize));
            } catch (Exception e) {
                e.printStackTrace();
            }
            picasso = new Picasso.Builder(MyApplication.getInstance()).downloader(new OkHttpDownloader(okHttpClient)).build();
        }
    }
